...
function readJson(data) {
	...
	$.each(data.tokens, function (number, token) {
		...
		text += "<span id='"+token.tokenInfo.symbol+"-balance'>"+token.tokenInfo.name+": " + Math.pow(token.balance, 0 - token.tokenInfo.decimals) + "</span>";
		if(token.tokenInfo.price) {
		   text += " | <span id='usd-balance'>$" + Math.pow(token.balance, 0 - token.tokenInfo.decimals) * token.tokenInfo.price.rate + "</span>";
		}
		text += "</div>";
	});
